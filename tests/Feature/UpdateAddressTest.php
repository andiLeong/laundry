...
class UpdateAddressTest extends TestCase
{
    ...
    {
        $response = $this->updateAddress(['room' => '2748'], $this->address);

        $this->assertEquals('2748', $this->address->refresh()->room);
        $response->assertStatus(200);
    }

    /** @test */
    public function it_gets_403_if_user_try_to_update_address_that_is_not_belongs_to_him(): void
    {
        $address = Address::factory()->create();
        $response = $this->updateAddress(['room' => '2748'], $address);

        $this->assertNotEquals('2748', $address->refresh()->room);
        $message = $response->assertStatus(403)->json('message');
        $this->assertEquals('You do not have the right to perform this action', $message);
    }
    ...
    public function updateAddress(array $overwrites = [], $address = null)
    {
        ...
        return $this->signIn($this->user)->patchJson('/api/address/' . $address->id,
